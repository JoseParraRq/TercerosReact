...
class TercerosController {
    ...
    async getOneTerceroByEmail(req, res) {

        try {
            var tercero = await new TercerosLogic().getOneTerceroByEmailLogic(req.body);
            console.log("here the users in the controller", tercero);
        } catch (error) {
            console.log("here the error in the get all users", error);
        }
        return res.json(tercero);
    }
    ...
    async loginController(req, res) {
        try {
            let tercero = await new TercerosLogic().getOneTerceroByEmailLogic(req.body);
            if (tercero[0] === undefined || tercero[0] === null) {
